...
And this is really important in order to stay up to date with the latest releases of each dependency and with build configuration updates, which believe me, we want to do in our projects since each release includes important bug fixes and improvements.
...
We'll find a [`packages/`](https://github.com/facebookincubator/create-react-app/tree/master/packages) directory in the repository that contains `npm` modules such as `react-scripts` (and `create-react-app` itself!).
...
One of the most important things I wanted to change in the default Webpack configuration was to [allow importing modules from the `src/` directory](https://github.com/aaccurso/create-react-app/commit/f31e9c8db2988e8722396ceabc22012f51c0d19a).
...
This is very useful when we are testing a new configuration, but we need to remember to release and publish when we get the job done!
